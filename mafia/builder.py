...
from mafia.constants import ROLE_CATEGORY_DESCRIPTIONS
from mafia.role import Role
from mafia.blob import TheBlob
from mafia.seer import Seer
from mafia.shifter import Shifter
from mafia.mafiagoon import MafiaGoon
from mafia.villager import Villager

log = logging.getLogger("red.justinard.mafia.builder")

# All roles in this list for iterating

ROLE_LIST = [TheBlob, Seer, Shifter, MafiaGoon, Villager]
...
def role_embed(idx, role: Role, color):
    ...
    embed.add_field(
        name="Alignment", value=["Town", "mafia", "Neutral"][role.alignment - 1], inline=False
    )
...
def say_role_list(code_list, rand_roles):
    ...
    for role in rand_roles:
        ...
        if 10 < role <= 16:
            role_dict[f"mafia {ROLE_CATEGORY_DESCRIPTIONS[role]}"] += 1
        if 20 < role <= 26:
    ...
    def setup(self):
        ...
        for k, v in ROLE_CATEGORY_DESCRIPTIONS.items():
            if 10 < k <= 19:
                ROLE_PAGES.append(
                    discord.Embed(
                        title="RANDOM:mafia Role",
                        description=f"mafia {v}",
