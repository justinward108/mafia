...
def mafialistener(name=None, priority=0):
    """A decorator that marks a function as a listener.

    This is the mafia.Game equivalent of :meth:`.Cog.listener`.
    ...
    def decorator(func):
        ...
        actual.__mafia_listener__ = priority
        to_assign = name or actual.__name__
        try:
            actual.__mafia_listener_names__.append((priority, to_assign))
        except AttributeError:
            actual.__mafia_listener_names__ = [(priority, to_assign)]
...
class MafiaListenerMeta(type):
    def __new__(mcs, *args, **kwargs):
        ...
        for base in reversed(new_cls.__mro__):
            for elem, value in base.__dict__.items():
                ...
                if inspect.iscoroutinefunction(value):
                    try:
                        is_listener = getattr(value, "__mafia_listener__")
                    except AttributeError:
                        continue
                    else:
        ...
        for listener in listeners.values():
            for priority, listener_name in listener.__mafia_listener_names__:
                # I use __name__ instead of just storing the value so I can inject
                # the self attribute when the time comes to add them to the bot
                listeners_as_list.append((priority, listener_name, listener.__name__))

        new_cls.__mafia_listeners__ = listeners_as_list
        return new_cls


class MafiaListener(metaclass=MafiaListenerMeta):
    def __init__(self, game):
        for priority, name, method_name in self.__mafia_listeners__:
