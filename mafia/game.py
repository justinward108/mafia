...
from mafia.builder import parse_code
from mafia.constants import ALIGNMENT_NEUTRAL
from mafia.player import Player
from mafia.role import Role
from mafia.votegroup import VoteGroup

log = logging.getLogger("red.justinward.mafia.game")

HALF_DAY_LENGTH = 259200  # FixMe: Make configurable
HALF_NIGHT_LENGTH = 86400
...
class Game:
    """
    Base class to run a single game of mafia
    """
    ...
    #         asyncio.ensure_future(self.village_channel.delete("mafia game-over"))
    #
    #     for c_data in self.p_channels.values():
    #         asyncio.ensure_future(c_data["channel"].delete("mafia game-over"))
    ...
    async def setup(self, ctx: commands.Context):
        ...
        if self.channel_category is None:
            self.channel_category = await self.guild.create_category(
                "mafia Game", overwrites=overwrite, reason="(BOT) New game of mafia"
            )
        else:  # No need to modify categories
            pass
            # await self.channel_category.edit(name="🔴 mafia Game (ACTIVE)", reason="(BOT) New game of mafia")
            # for target, ow in overwrite.items():
            #     await self.channel_category.set_permissions(target=target,
            #                                                 overwrite=ow,
            #                                                 reason="(BOT) New game of mafia")
            ...
            try:
                for target, ow in overwrite.items():
                    ...
                    await self.village_channel.set_permissions(
                        target=target, overwrite=curr, reason="(BOT) New game of mafia"
                    )
            except discord.Forbidden:
                await ctx.maybe_send_embed(
    ...
    async def _at_day_start(self):  # ID 1
        ...
        # await self.village_channel.edit(reason="WW Night Start", name="mafia-🌞")
    ...
    async def _notify(self, event_name, **kwargs):
        for i in range(1, 7):  # action guide 1-6 (0 is no action)
            ...
            # self.bot.dispatch(f"red.fox.mafia.{event}", data=data, priority=i)
    ...
    async def _end_game(self):
        ...
        try:
            asyncio.create_task(self.village_channel.edit(reason=reason, name="mafia"))
    ...
    # def remove_mafia_listener(self, func, name=None):
